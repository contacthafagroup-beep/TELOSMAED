...
export function BookPromo() {
  ...
  const handlePreOrder = async (e: React.FormEvent) => {
    e.preventDefault()
    // Simple notify - can be wired to newsletter API later
    setNotified(true)
    setShowForm(false)
  }

  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#0d4a3a] via-[#0f5a45] to-[#0d4a3a] py-10 sm:py-14 lg:py-20">
      {/* Background pattern */}
      <div className="absolute inset-0 opacity-10"
        style={{
          backgroundImage: 'radial-gradient(circle at 20% 50%, rgba(255,255,255,0.15) 1px, transparent 1px), radial-gradient(circle at 80% 50%, rgba(255,255,255,0.15) 1px, transparent 1px)',
      ...
      <div className="relative container-responsive">
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16">
          ...
          >
            {/* Coming Soon Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-[#F4C430]/20 border border-[#F4C430]/40 rounded-full mb-4">
            ...
            {/* Author */}
            <p className="text-[#F4C430] text-base sm:text-lg font-medium mb-4">
              በ ብርሃኑ ተስፋዬ
            </p>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 rounded-lg mb-8 border border-white/20">
              <CalendarIcon className="h-4 w-4 text-[#F4C430]" />
            ...
            {!notified ? (
                ...
                {!showForm ? (
                  <>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                      className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#F4C430] text-[#0d4a3a] font-bold rounded-xl shadow-lg hover:bg-[#f0b800] transition-all duration-200 text-sm sm:text-base min-h-[48px]"
                    >
                    ...
                    <motion.button
                      ...
                      whileHover={{ scale: 1.03 }}
                      whileTap={{ scale: 0.97 }}
                      className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white/10 text-white font-semibold rounded-xl border border-white/30 hover:bg-white/20 transition-all duration-200 text-sm sm:text-base min-h-[48px]"
                    >
                  ...
                  <form onSubmit={handlePreOrder} className="flex flex-col sm:flex-row gap-2 w-full max-w-md mx-auto lg:mx-0">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-3 bg-[#F4C430] text-[#0d4a3a] font-bold rounded-xl hover:bg-[#f0b800] transition-all duration-200 text-sm min-h-[48px] whitespace-nowrap"
                    >
                      Notify Me
                    </button>
                  </form>
              ...
              <motion.div
                ...
                className="inline-flex items-center gap-2 px-5 py-3 bg-[#F4C430]/20 border border-[#F4C430]/40 rounded-xl text-[#F4C430] font-semibold text-sm"
              >
